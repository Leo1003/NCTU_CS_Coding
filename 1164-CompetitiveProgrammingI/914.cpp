...
struct state {
    ...
    state(long long panel)
        : panel(panel), Wx(-1), Wy(-1)
    {
        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < 5; j++) {
                if (this->get(i, j) == 0) {
...
void dump_states(long long walked[5][5])
{
    for (int i = 0; i < 5; i++) {
        for (int j = 0; j < 5; j++) {
            fprintf(stderr, "%016llx ", walked[i][j]);
        }
        fprintf(stderr, "\n");
    }
}

void dump_panel(long long panel)
{
    state s(panel);
    for (int i = 0; i < 5; i++) {
        for (int j = 0; j < 5; j++) {
